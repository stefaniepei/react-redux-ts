...
module.exports = {
  ...
  module: { //定义了对模块的处理逻辑，这里可以用loaders定义了一系列的加载器，以及一些正则。当需要加载的文件匹配test的正则时，就会调用后面的loader对文件进行处理，这正是webpack强大的原因。比如这里定义了凡是 .js 结尾的文件都是用 babel-loader 做处理，而 .jsx 结尾的文件会先经过 jsx-loader 处理，然后经过 babel-loader 处理。当然这些loader也需要通过 npm install 安装
    loaders: [{
      ...
      test: /\.scss$/,
      loader: ExtractTextPlugin.extract("style-loader", "css-loader!autoprefixer-loader?{browers:['last 2 version', 'Firefox 15']}!sass-loader?outputStyle=expanded")
    }, {
      test: /\.json$/,
      loader: 'json-loader'
    }, {
  ...
  resolve: { //定义了解析模块路径时的配置，常用的就是extensions，可以用来指定模块的后缀，这样在引入模块时就不需要写后缀了，会自动补全
    ...
    alias: {
      'tools': path.join(nodeModulePath, '/jquery/dist/jquery.min'),
      'light7js': path.join(nodeModulePath, '/light7/dist/js/light7.min'),
      'light7css': path.join(nodeModulePath, '/light7/dist/css/light7.min.css'),
